const Lexer = require("../lexer-based-on-regex");
...
describe("#lexer-based-on-regex", () => {
    describe("test basic matchers", () => {
        ...
        it("", () => {
            assert.strictEqual(lexer.lex(), "X");
        });
        it("", () => {
            assert.strictEqual(lexer.lex(), "X");
        });
        it("", () => {
            assert.strictEqual(lexer.lex(), "Y");
        });
        it("", () => {
            assert.strictEqual(lexer.lex(), "X");
        });
        it("should be EOF", () => {
            assert.strictEqual(lexer.lex(), "EOF");
        });
    });
    ...
    describe("宏+复杂处理函数", () => {
        ...
        let lexer = new Lexer(dict, input);
        it("{", () => {
            assert.strictEqual(lexer.lex(), "l_brace");
        });
        it("string", () => {
            assert.strictEqual(lexer.lex(), "STRING");
        });
        it(";", () => {
            assert.strictEqual(lexer.lex(), "semi");
        });
        it("number", () => {
            assert.strictEqual(lexer.lex(), "NUMBER");
        });
        it(",", () => {
            assert.strictEqual(lexer.lex(), "comma");
        });
        it("}", () => {
            assert.strictEqual(lexer.lex(), "r_brace");
        });
    });
    describe("c minus", () => {
        let dict = {
    ...
    if((aa%2==1.2+2&&aa==12)||(aa%2==0&&(aa>0||aa==-1)))
        ...
        it("", () => {
            assert.strictEqual(str, `program test { 
// 基本功能 
int a [ 10 ] [ 20 ] [ 5 ] , i , j , k ; 
float ? b , ? c = 1.2 ; 
int aa = 2 ; 
