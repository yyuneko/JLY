...
describe("test SLR", () => {
    ...
    describe("# test calculator", () => {
        ...
        it("1+2/5*3=2.2", () => {
            assert.strictEqual(parser.parse("1+2/5*3"), 2.2);
        })
        it("(1.3+2)*10%2=1", () => {
            assert.strictEqual(parser.parse("(1.3+2)*10%2"), 1);
        })
        it("56/(2*3.5)=8", () => {
            assert.strictEqual(parser.parse("56/(2*3.5)"), 8);
        })
        it("-23*2e2/6=-920", () => {
            assert.strictEqual(parser.parse("-23*2e2/5"), -920)
        })
    })
    describe("# test 2", () => {
        let grammar = {
...
First:\n\t${Reflect.ownKeys(parser.first).map(key => {
                return key.toString() + ": " + parser.first[key].map(item => item.toString()).join(", ");
            }).join('\n\t')}

Follow:\n\t${Reflect.ownKeys(parser.follow).map(key => {
                return key.toString() + ": " + parser.follow[key].map(item => item.toString()).join(", ");
            }).join('\n\t')}
